...
class DCAAgentTerminateConstantLevel(SPAgent):
    def __init__(self, model, id, historical_power, start_date, end_date,
                 max_sealing_throughput=constants.DEFAULT_MAX_SEALING_THROUGHPUT_PIB, max_daily_rb_onboard_pib=3, 
                 renewal_rate=0.6, fil_plus_rate=0.6, sector_duration=365, debug_mode=False, 
                 terminate_start_date=None, N_duration_history_terminate=None):
        super().__init__(model, id, historical_power, start_date, end_date, max_sealing_throughput_pib=max_sealing_throughput)
        ...
        self.terminate_start_date = terminate_start_date
        # determines which date of power we will terminate
        self.N_duration_history_terminate = N_duration_history_terminate
        if self.N_duration_history_terminate is None:
            self.N_duration_history_terminate = self.sector_duration // 2
        assert self.terminate_start_date >= (self.start_date + timedelta(days=self.N_duration_history_terminate))

    def step(self):
        if self.terminate_start_date is not None and self.current_date >= self.terminate_start_date:
            onboarding_date_to_terminate = self.current_date - timedelta(days=self.N_duration_history_terminate)
            # print('Terminating Power onboarded on:', onboarding_date_to_terminate, 'Current Date', self.current_date)
            self.terminate_modeled_power(onboarding_date_to_terminate)

        # continue w/ onboarding and renewals
        rb_to_onboard = min(self.max_daily_rb_onboard_pib, self.max_sealing_throughput_pib)
        qa_to_onboard = self.model.apply_qa_multiplier(rb_to_onboard * self.fil_plus_rate,
                                                    fil_plus_multipler=constants.FIL_PLUS_MULTIPLER,
                                                    date_in=self.current_date,
